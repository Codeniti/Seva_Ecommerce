...
var alogin = new aLogin();

function aLogin()
{
  var aloginFlag = false;
  
  
  this.asetLoginFlag = asetLoginFlag;
  this.agetLoginFlag = agetLoginFlag;
  this.acallURL = acallURL;
  this.asocialnetwork = asocialnetwork;
  this.aopenSignIn = aopenSignIn;
  this.acloseSignIn = acloseSignIn;
  this.ainitSignInForm = ainitSignInForm;  
  this.aopenSignInForm = aopenSignInForm;
  this.aopenRegistrationForm = aopenRegistrationForm;
  this.aopenForgotPasswordForm = aopenForgotPasswordForm;
  this.aopenTermConditionForm = aopenTermConditionForm;
  this.asubmitSignInForm = asubmitSignInForm;
  this.asubmitRegistrationForm = asubmitRegistrationForm;
  this.asubmitForgotPasswordForm = asubmitForgotPasswordForm;
  this.acheckEmailExists = acheckEmailExists;
  this.acallSubmitSignInForm = acallSubmitSignInForm;
  this.asetShoppingCartCount = asetShoppingCartCount
  

  function acallSubmitSignInForm(event)
  {
	var keyCode;
	if(window.event)
		keyCode =window.event.keyCode;
  ...
  function asetLoginFlag(flag)
  {
  	aloginFlag = flag;  	
  }
  
  function agetLoginFlag()
  {
  	return aloginFlag;  	
  }
  ...
  function asocialnetwork(providerId)
  {
	window.open ('/faces/jsp/components/thirdparty/socialnetwork/socialnetwork.jsp?id='+providerId,"socialnetworklogin","status=1,width=400,height=400,left=300,top=300,resizable=1"); 
  }
  
  function aopenSignIn()
  {
		$('#alogin-panel-link').trigger('click');
  }
  
  function acloseSignIn()
  {
  		$.fancybox.close();
  }
  
  function ainitSignInForm()
  {
	$("#alogin-panel-link").fancybox({	//Login Popup
					'titlePosition'		: 'inside',
					'transitionIn'		: 'none',
					'transitionOut'		: 'none'
			    });
  ...
  function acloseSignInForm()
  {  
     $("#alogin-panel-form").get(0).reset();     
     $("#alogin-panel-message").empty();
  	 $("#login-panel").hide();
  }
  
  
  function aopenRegistrationForm()
  {
  	acloseAllSignInForm();
  ...
  function acloseRegistrationForm()
  {  
     $("#registration-panel-form").get(0).reset();     
     $("#registration-panel-message").empty();
  	 $("#registration-panel").hide();
  ...
  function aopenTermConditionForm()
  {
  	//var termConditionInfoLength = $("#termConditionInfo").html().length;
  	if($("#termConditionInfo").html() == null  ||  ( $("#termConditionInfo").html() !=null && $("#termConditionInfo").html().length < 10) )
  	{
  ...
  function acloseTermConditionForm()
  {
  	$("#termCondition-panel").hide();
  }
  
  function aopenForgotPasswordForm()
  {
    $("#forgotpassword-panel-form").get(0).reset();     
    $("#forgotpassword-panel-message").empty();
  	$("#forgotpassword-panel").slideToggle();
  ...
  function acloseForgotPasswordForm()
  {
    $("#forgotpassword-panel-form").get(0).reset();     
    $("#forgotpassword-panel-message").empty();
  	$("#forgotpassword-panel").hide();
  ...
  function acloseAllSignInForm()
  {
    acloseForgotPasswordForm();
    acloseTermConditionForm();
  	acloseSignInForm();
  	acloseRegistrationForm();
  }
  
  function asubmitSignInForm()
  {
		var emailId = $("#alogin-panel-emailid").val();
		var password = $("#alogin-panel-password").val();
		
		if(emailId =="")
		{	
			    $("#alogin-panel-message").html("<span class='errMsg'> <span class='icn_Error'>&nbsp;</span>Oops! Looks like you forgot to enter your email ID</span>");
			    return false;
		}
		else
		{
		var testPattern = function(value, pattern) 
			...
			if(!isvalidemail)
			{
				$('#alogin-panel-message').html(" <span class='errMsg'>  <span class='icn_Error'>&nbsp;</span>Please Enter a Valid E-mail Id.</span>");
				return false;
			}
		}
		...
		if(password =="")
		{	
			    $("#alogin-panel-message").html("<span class='errMsg'> <span class='icn_Error'>&nbsp;</span>There is no password typed! Please type your password</span>");
			    return false;
		}
		
		
				$("#alogin-panel-message").html("<span class='waitMsg'>Please wait few Seconds... </span>");
				$.post('/faces/jsp/ajax/ajax.jsp',{actionname : "doLogin", emailId : emailId , password : password } ,function(data)
				{
					if(data.status == 'success')
					{
					...
					{
						$('#alogin-panel-message').html(" <span class='errMsg'> <span class='icn_Error'>&nbsp;</span>Looks like the E-mail id or password is wrong! Please Try again.</span>");
					}
				},"json")
				.error(function() 
				{ 
					$('#alogin-panel-message').html(" <span class='errMsg'> <span class='icn_Error'>&nbsp;</span> Error Occured Please Try Again !</span>");
				});
			 
  }
  
  function asubmitRegistrationForm()
  {
		var email = $("#registration-panel-email").val();
		var password = $("#registration-panel-password").val();
		var repeatPass=$("#registration-panel-repeat-password").val();	
  ...
  function asubmitForgotPasswordForm()
  {
		var emailId = $("#forgotpassword-panel-email").val();
		if(emailId=="")
		{	
...
$(document).ready(function () 
{
	login.ainitSignInForm();
	socialNetworkApps.loadHomePageSocialNetworkLinks();
	visitor.ainitVisitorForm();
});
